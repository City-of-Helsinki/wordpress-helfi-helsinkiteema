...
function helsinkiGalleryLightbox(config) {
  ...
  function initLightbox(gallery) {
    var figures = galleryFigures(gallery);
    if (figures.length === 0) {
      return;
    }
    ...
    var lightbox = createLightbox(gallery, figures);
  ...
  function initGalleryImage(figure) {
    var imageLink = figure.querySelector('a');
    if (imageLink && imageLink.querySelector('img')) imageLink.addEventListener('click', clickImage);
  }
  ...
  function clickImage(event) {
    event.preventDefault();
    openLightBox(currentGalleryLightBox(currentGallery(event.currentTarget)), event.currentTarget.firstElementChild, event.currentTarget);
  }
  ...
  function clickNext(event) {
    ...
    var currentImage = lightboxActiveImage(currentActive);
    if (currentImage) {
  ...
  function openLightBox(lightbox, clickedImage, clickedElement) {
    ...
    var currentActiveImage = lightboxActiveImage(lightbox),
  ...
  function switchActiveImage(currentImage, newImage) {
    ...
    if (newImage) {
      ...
      adjustContainerMaxWidth(currentGalleryContent(newImage), newImage.firstElementChild.naturalWidth);
    }
  }
  ...
  function currentGalleryLightBox(gallery) {
    return document.getElementById(gallery.getAttribute('data-lightbox-id'));
  }
  ...
  function lastLightboxImage(lightbox) {
    var figures = lightbox.querySelectorAll('.lightbox__images figure');
    return figures[figures.length - 1];
  }
  function lightboxFigure(lightbox, source) {
    var img = lightbox.querySelector('.lightbox__images img[src="' + source + '"]');
  ...
  function createLightbox(gallery, figures) {
    // Element
    var lightboxWrap = createContainer('div', ['lightbox']);
    var content = createContainer('div', ['lightbox__content']);
    var title = createHeading('h2', strings.lightboxTitle);

    // Attributes
    var lightboxId = generateLightboxId(gallery);
  ...
  function createCloseButton(lightboxId) {
    // Element
    var button = createContainer('button', ['lightbox__close', 'button-reset']),
  ...
  function createNavigation() {
    var container = createContainer('div', ['lightbox__navigation']);
  ...
  function createNavigationItem(type, icon, text, onClick) {
    // Element
    var button = createContainer('button', ['lightbox__' + type, 'hds-button', 'hds-button--small']),
      screenReaderText = createContainer('span', ['screen-reader-text']);
  ...
  function createImages(figures) {
    var container = createContainer('div', ['lightbox__images']);
    for (var f = 0; f < figures.length; f++) {
...
var HelsinkiGalleryLightbox = helsinkiGalleryLightbox(helsinkiTheme);
HelsinkiGalleryLightbox.init(document.querySelectorAll('.wp-block-gallery, .gallery'), document.querySelectorAll(':not(.wp-block-gallery, .gallery) > .wp-block-image, .wp-caption'));
...
(function () {
  var headerSearch = HeaderSearch(getHeaderSearchElements());
  if (headerSearch) {
  ...
  function HeaderSearch(elements) {
    ...
    function _isOffClick(event) {
      return !_elementIsOrContains(elements.container, event.target) && !_elementIsOrContains(elements.form, event.target) && !_elementIsOrContains(elements.toggle, event.target);
    }
    function _isFocusOut(event) {
      return event.relatedTarget && _isInFocus && !_elementIsOrContains(elements.form, event.relatedTarget) && !_elementIsOrContains(elements.toggle, event.relatedTarget);
    }
  ...
  function getHeaderSearchElements() {
    var _container = document.getElementById('header-search');
    if (!_container) {
      return;
    }
    var _toggle = document.getElementById(_container.getAttribute('aria-labelledby'));
    var _form = _container.querySelector('.search-form');
    if (!_toggle || !_form) {
      return;
    }
    return {
...
function jsSidebarNavInit() {
  ...
  var sidebarNavToggle = document.querySelectorAll('.js-sidebarnavigation-toggle');
  ...
  forCallbackLoop(sidebarNavToggle, function (sidebarMenuToggle) {
    ...
    if (menuItem.classList.contains('current-menu-item') || menuItem.classList.contains('current-menu-ancestor') || menuItem.classList.contains('current_page_ancestor') || menuItem.classList.contains('current_page_parent')) {
...
function ifControlsNoScroll(element) {
  if (element.classList.contains('js-toggle-no-scroll')) {
    ...
    if ('up' === limit && screen.width > breakpoint || screen.width < breakpoint) {
      return true;
    } else {
      return false;
    }
