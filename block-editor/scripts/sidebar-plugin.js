( function( wp ) {
  ...
  const { withSelect, withDispatch, subscribe, select } = wp.data;
  const { Panel, PanelBody, PanelRow, TextControl, ToggleControl, SelectControl } = wp.components;
	...
	const toggleField = function( props ) {
		return createElement( ToggleControl, {
			...
			onChange: toggled => props.setMetaFieldValue( toggled ? true : false ),
		});
	}
    ...
    return createElement( SelectControl, {
      ...
      options: selectOptions.filter(function(option){
        return displayHeroStyles;
			...
			onChange: selected => props.setMetaFieldValue( selected ),
		});
  }
  ...
  function createSelectOptions( data ) {
	...
	const logoIcon = createElement('svg', { height: 24, viewBox: '0 0 78 36', }, createElement( 'path', {
    ...
    },
    {
      ...
      rows: [
        ...
        },
        {
          condition: true,
          type: TextMetaField,
          config: { fieldLabel: 'Button 1 text', fieldType: 'text', fieldName: 'hero_cta_text' }
        },
        {
          condition: true,
          type: TextMetaField,
        ...
        },
        {
          condition: true,
          type: TextMetaField,
          config: { fieldLabel: 'Button 2 text', fieldType: 'text', fieldName: 'hero_cta_2_text' }
        },
      ],
    },
    {
  ...
  function createMetaControls( config ) {
    ...
    for (var c = 0; c < config.length; c++) {
      ...
      for (var r = 0; r < config[c].rows.length; r++) {
        ...
        rows.push(
          createElement( config[c].rows[r].type, config[c].rows[r].config )
        );
      }

      if ( rows.length > 0 ) {
        elements.push(
          createElement( PanelBody, { title: config[c].title, initialOpen: false }, rows )
        );
      }
    }
