...
<div class="hds-pagination-container">
  <nav class="hds-pagination" role="navigation" aria-label="<?php esc_html_e( 'Page navigation', 'helsinki-universal' ); ?>" data-next="<?php esc_html_e( 'Next', 'helsinki-universal' ); ?>">
    ...
    <?php if ( $paged > 1 ) : ?>
      <a class="previous hds-button hds-button--supplementary hds-pagination__button-prev" href="<?php echo esc_url( get_pagenum_link( $paged - 1 ) ); ?>" aria-label="<?php esc_html_e( 'Previous page', 'helsinki-universal' ); ?>"><span><?php echo helsinki_get_svg_icon('angle-left', 'inline-icon'); ?></span><span class="hds-button__label"><?php esc_html_e( 'Previous', 'helsinki-universal' ); ?></span></a>
    <?php endif; ?>

    <ul class="hds-pagination__pages">

    <?php if ( $paged >= $showitems -1 ) : ?>
      <li><a href="<?php echo esc_url( get_pagenum_link( 1 ) ); ?>" class="hds-pagination__item-link" aria-label="<?php esc_html_e( 'First page', 'helsinki-universal' ); ?>"><span><?php esc_html_e(1); ?></span></a></li>
    ...
    <?php if ( $paged < $pages - $range ) : ?>
      <li><span class="hds-pagination__item-ellipsis">...</span></li>
      <li><a href="<?php echo esc_url( get_pagenum_link( $pages ) ); ?>" class="hds-pagination__item-link" aria-label="<?php esc_html_e( 'Last page', 'helsinki-universal' ); ?>"><span><?php esc_html_e($pages); ?></span></a></li>
    <?php endif; ?>

    </ul>

    <?php if ( $paged < $pages ) : ?>
      <a class="next hds-button hds-button--supplementary hds-pagination__button-next" href="<?php echo esc_url( get_pagenum_link( $paged + 1 ) ); ?>"  aria-label="<?php esc_html_e( 'Next page', 'helsinki-universal' ); ?>"><span class="hds-button__label"><?php esc_html_e( 'Next', 'helsinki-universal' ); ?></span><span><?php echo helsinki_get_svg_icon('angle-right', 'inline-icon'); ?></span></a>
    <?php endif; ?>
